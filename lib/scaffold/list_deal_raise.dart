...
import 'package:ptnsupplier/scaffold/detail_deal_raise.dart';
...
class ListDealRaise extends StatefulWidget {
  // ListDealRaise
  final int? index;
  final UserModel? userModel;
  final DealraiseModel? dealModel;

  ListDealRaise({Key? key, this.index, this.userModel, this.dealModel})
      : super(key: key);

  @override
  _ListDealRaiseState createState() => _ListDealRaiseState();
}
...
class _ListDealRaiseState extends State<ListDealRaise> {
  // Explicit
  int? myIndex;
  UserModel? myUserModel;
  List<DealraiseModel> allDealModels = []; // set array
  List<DealraiseModel> filterDealModels = [];
  ...
  Future<void> readData() async {
    ...
    String? url =
        'https://ptnpharma.com/apisupplier/json_deal_raise.php?memberId=$memberId&searchKey=$searchString&page=$page&sort=$sort';
    http.Response response = await http.get(Uri.parse(url));
    var result = json.decode(response.body);
    print('url = $url');
    var itemDeal = result['itemsData'];
    for (var map in itemDeal) {
      DealraiseModel dealModel = DealraiseModel.fromJson(map);
      String? subject = dealModel.med!;
      DealraiseModel allDealModel = DealraiseModel.fromJson(map);
      setState(() {
      ...
      children: [
        ...
        Column(
          children: [
            ...
            Text('ทุน'),
            Text(
              filterDealModels[index].newprice!,
              style: TextStyle(
                fontSize: 16.0,
                fontWeight: FontWeight.bold,
                color: Color.fromARGB(0xff, 0, 0, 0),
      ...
      children: [
        Column(
          children: [
            ...
            Text('วันที่ปรับราคา'),
            Text(
              //filterDealModels[index].actiondate!,
              DateFormat('dd/MM/yyyy').format(DateFormat('yyyy-MM-dd').parse(filterDealModels[index].actiondate!)),
              style: TextStyle(
                fontSize: 14.0,
                fontWeight: FontWeight.bold,
                color: Color.fromARGB(0xff, 0, 0, 0),
        ...
        Container(
                  width: 35.0,
                  child: (filterDealModels[index].dealstatus=='0')?Image.asset('images/icon_edit.png'):null,
                )
      ],
    );
    // return Text('na');
          ...
          child: SizedBox(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                  textStyle: const TextStyle(fontSize: 14),side: BorderSide(
                    color:  Color.fromRGBO(44, 174, 235, 1),
              ...
              child: const Text('+ เสนอดีล',style: TextStyle(color:Color.fromRGBO(44, 174, 235, 1), ),),
            ),
          ),
        ),
        Container(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('รายการแจ้งปรับราคาสินค้า'),
        actions: <Widget>[
          showHome(),
        ],
      ),
...
class _WebViewExampleState extends State<WebViewExample> {
  ...
  void initState() {
    ...
    String? urlView =
        'https://ptnpharma.com/supplier/pages/forms/dealraise_mobile.php?mode=v&memberId=$memberId'; //
...
class _WebViewFromDealState extends State<WebViewFromDeal> {
  ...
  Widget build(BuildContext context) {
    ...
    return WebviewScaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.lightBlue
        title: Text("ฟอร์มแจ้งปรับราคาสินค้า"),
      ),
      withZoom: true,
      withJavascript: true,
