...
import 'package:ptnsupplier/scaffold/detail_deal_closesale.dart';
...
class ListDealClosesale extends StatefulWidget {
  // ListDealClosesale
  final int? index;
  final UserModel? userModel;
  final DealclosesaleModel? dealModel;

  ListDealClosesale({Key? key, this.index, this.userModel, this.dealModel})
      : super(key: key);

  @override
  _ListDealClosesaleState createState() => _ListDealClosesaleState();
}
...
class _ListDealClosesaleState extends State<ListDealClosesale> {
  // Explicit
  int? myIndex;
  UserModel? myUserModel;
  List<DealclosesaleModel> allDealModels = []; // set array
  List<DealclosesaleModel> filterDealModels = [];
  ...
  Future<void> readData() async {
    ...
    String? url =
        'https://ptnpharma.com/apisupplier/json_deal_closesale.php?memberId=$memberId&searchKey=$searchString&page=$page&sort=$sort';
    http.Response response = await http.get(Uri.parse(url));
    var result = json.decode(response.body);
    print('url = $url');
    var itemDeal = result['itemsData'];
    for (var map in itemDeal) {
      DealclosesaleModel dealModel = DealclosesaleModel.fromJson(map);
      String? subject = dealModel.med;
      DealclosesaleModel allDealModel = DealclosesaleModel.fromJson(map);
      setState(() {
  ...
  Widget showName(int? index) {
    String? stringMed = filterDealModels[index!].med;
    ...
    return Column(
      children: <Widget>[
        ...
        Container(
          ...
          child: Text(
            (stringMed!.length < 80) ? stringMed : stringMed.substring(0, 80), //
      ...
      children: [
        ...
        Column(
          children: [
            ...
            Text('ยอดที่สั่งแล้ว'),
            Text(
              filterDealModels[index].currentorder!,
              style: TextStyle(
                fontSize: 16.0,
                fontWeight: FontWeight.bold,
                color: Color.fromARGB(0xff, 0, 0, 0),
        ...
        //     Text('ยอดต้องสั่งเพิ่ม'),
        //     Text(
        //       filterDealModels[index!].balance,
        //       style: TextStyle(
        //         fontSize: 16.0,
        //         fontWeight: FontWeight.bold,
        //         color: Color.fromARGB(0xff, 0, 0, 0),
